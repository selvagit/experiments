...
void AlaLed::setBrightness(byte maxOut) { this->maxOut = maxOut; }
...
void AlaLed::setAnimation(AlaSeq animSeq[])
{
  ...
  animSeqDuration = 0;
  for (animSeqLen = 0; animSeq[animSeqLen].animation != ALA_ENDSEQ;
       animSeqLen++)
  {
...
int AlaLed::getAnimation() { return animation; }
...
bool AlaLed::runAnimation()
{
  ...
  if (driver == ALA_PWM)
  {
    for (int i = 0; i < numLeds; i++) analogWrite(pins[i], leds[i]);
  }
  else if (driver == ALA_TLC5940)
  {
    for (int i = 0; i < numLeds; i++) Tlc.set(pins[i], leds[i] * 16);
...
void AlaLed::setAnimationFunc(int animation)
{
  switch (animation)
  {
    ...
    case ALA_OFF:
      ...
      break;
    case ALA_BLINK:
      animFunc = &AlaLed::blink;
      break;
    case ALA_BLINKALT:
      animFunc = &AlaLed::blinkAlt;
      break;
    case ALA_SPARKLE:
      animFunc = &AlaLed::sparkle;
      break;
    case ALA_SPARKLE2:
      animFunc = &AlaLed::sparkle2;
      break;
    case ALA_STROBO:
      animFunc = &AlaLed::strobo;
      break;

    case ALA_PIXELSHIFTRIGHT:
      animFunc = &AlaLed::pixelShiftRight;
      break;
    case ALA_PIXELSHIFTLEFT:
      animFunc = &AlaLed::pixelShiftLeft;
      break;
    case ALA_PIXELBOUNCE:
      animFunc = &AlaLed::pixelBounce;
      break;
    case ALA_PIXELSMOOTHSHIFTRIGHT:
      animFunc = &AlaLed::pixelSmoothShiftRight;
      break;
    case ALA_PIXELSMOOTHSHIFTLEFT:
      animFunc = &AlaLed::pixelSmoothShiftLeft;
      break;
    case ALA_PIXELSMOOTHBOUNCE:
      animFunc = &AlaLed::pixelSmoothBounce;
      break;
    case ALA_COMET:
      animFunc = &AlaLed::comet;
      break;
    case ALA_BARSHIFTRIGHT:
      animFunc = &AlaLed::barShiftRight;
      break;
    case ALA_BARSHIFTLEFT:
      animFunc = &AlaLed::barShiftLeft;
      break;
    case ALA_LARSONSCANNER:
      animFunc = &AlaLed::larsonScanner;
      break;
    case ALA_LARSONSCANNER2:
      animFunc = &AlaLed::larsonScanner2;
      break;

    case ALA_FADEIN:
      animFunc = &AlaLed::fadeIn;
      break;
    case ALA_FADEOUT:
      animFunc = &AlaLed::fadeOut;
      break;
    case ALA_FADEINOUT:
      animFunc = &AlaLed::fadeInOut;
      break;
    case ALA_GLOW:
      animFunc = &AlaLed::glow;
      break;
    case ALA_FLAME:
      animFunc = &AlaLed::flame;
      break;
...
void AlaLed::comet()
{
  ...
  for (int x = 0; x < numLeds; x++)
  {
    leds[x] = (int)constrain(
        (((x - t) / l + 1.2f)) * (((x - t) < 0) ? 1 : 0) * maxOut, 0, maxOut);
  }
}
...
void AlaLed::pixelSmoothBounce()
{
  ...
  for (int x = 0; x < numLeds; x++)
  {
    leds[x] = constrain((int)((-abs(abs(t - numLeds + 1) - x) + 1) * maxOut), 0,
                        maxOut);
  }
}
...
void AlaLed::larsonScanner()
{
  ...
  for (int x = 0; x < numLeds; x++)
  {
    leds[x] = constrain((int)((-abs(abs(t - numLeds + 1) - x) + l) * maxOut), 0,
                        maxOut);
  }
}
...
void AlaLed::larsonScanner2()
{
  ...
  {
    leds[x] =
        constrain((int)((-abs(abs(t - numLeds - 2 * l) - x - l) + l) * maxOut),
                  0, maxOut);
  }
}
