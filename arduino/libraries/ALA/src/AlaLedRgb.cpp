...
void AlaLedRgb::setBrightness(AlaColor maxOut) { this->maxOut = maxOut; }
...
int AlaLedRgb::getCurrentRefreshRate() { return refreshRate; }
                             ...
                             bool isSeq)
{
  ...
  if (this->animation == animation && this->speed == speed
      && this->palette.numColors == 1 && this->palette.colors[0] == color)
    return;
...
void AlaLedRgb::setColor(AlaColor color) { this->palette.colors[0] = color; }

int AlaLedRgb::getAnimation() { return animation; }
...
bool AlaLedRgb::runAnimation()
{
  ...
  else if (driver == ALA_WS2812)
  {
    ...
    for (int i = 0; i < numLeds; i++)
      neopixels->setPixelColor(i,
                               neopixels->Color((leds[i].r * maxOut.r) >> 8,
                                                (leds[i].g * maxOut.g) >> 8,
                                                (leds[i].b * maxOut.b) >> 8));
...
void AlaLedRgb::setAnimationFunc(int animation)
{
  switch (animation)
  {
    ...
    case ALA_BOUNCINGBALLS:
      animFunc = &AlaLedRgb::bouncingBalls;
      break;
    case ALA_BUBBLES:
      animFunc = &AlaLedRgb::bubbles;
      break;
...
void AlaLedRgb::cometCol()
{
  ...
  {
    float tx =
        mapfloat(max(t - x, 0), 0, numLeds / 1.7, 0, palette.numColors - 1);
...
void AlaLedRgb::pixelSmoothBounce()
{
  ...
  AlaColor c = palette.getPalColor(
      getStepFloat(animStartTime, speed, palette.numColors));
...
void AlaLedRgb::plasma()
{
  ...
  {
    AlaColor c1 =
        palette.getPalColor((float)((x + t) * palette.numColors) / numLeds);
    AlaColor c2 = palette.getPalColor(
        (float)((2 * x - t + numLeds) * palette.numColors) / numLeds);
...
void AlaLedRgb::movingBars()
{
  ...
  for (int x = 0; x < numLeds; x++)
  {
    leds[x] = palette.colors[(((t + x) * palette.numColors) / numLeds)
                             % palette.numColors];
  }
}
...
void AlaLedRgb::bouncingBalls()
{
  ...
  for (int i = 0; i < palette.numColors; i++)
  {
    if (pxSpeed[i] > -0.04 and pxSpeed[i] < 0 and pxPos[i] > 0
        and pxPos[i] < 0.1)
...
void AlaLedRgb::bubbles()
{
  ...
  for (int i = 0; i < palette.numColors; i++)
  {
    ...
    {
      int p      = mapfloat(pxPos[i], 0, 1, 0, numLeds - 1);
      AlaColor c = palette.colors[i].scale(
          1 - (float)random(10) / 30); // add a little flickering
      leds[p] = c;
    }
  }
}
