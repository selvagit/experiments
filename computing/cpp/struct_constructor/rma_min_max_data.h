...
struct __attribute__((__packed__)) RMAMinMaxData {
  ...
  RMAMinMaxData()
      : _max(0), _min(100), _avg(0), _frame_size(ONE_MIN), _curr_index(1) {
    std::cout << "inside the constructor RMAMinMaxData " << std::endl;
    _data = 0;
  }

  RMAMinMaxData(int frame_size)
      : _max(0), _min(100), _avg(0), _frame_size(frame_size), _curr_index(1) {
    _data = 0;
  }
  ...
  void insert(uint8_t curr_sample) {
    ...
    // RMA_PRINT_CONSOLE("'Modi'fied _curr = %d _data(lower) = %x _data(higher)
    // = %x_curr_index = %d \r\n",_curr,_lower,_higher,_curr_index);
    std::cout << "'Modi'fied _curr = " << static_cast<unsigned int>(_curr)
              << " _data(lower) =" << _lower << " data(higher) = " << _higher
              << " _curr_index = " << _curr_index << std::endl;
  ...
  uint8_t get_max() { return _max; }

  uint8_t get_min() { return _min; }

  uint8_t get_avg() { return _avg; }

  uint8_t get_curr() { return _curr; }
};
