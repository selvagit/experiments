...
/* use this macro if the function where this macro
 * is called must return a value */
#define ARG_CHECK_RET_X(cond, X) ASSERT_RET_X(cond, X, "Input Argument error");

/* use this macro if the function where this macro
 * is called must return a value */
#define ARG_CHECK_RET(cond, X) ASSERT_RET(cond, X, "Input Argument error");

static inline void check(int line, char* file_name, int value) {
  if (!value) printf("error %d, %s", line, file_name);
}

#define ARG_CHK(X) check(__LINE__, __FILE__, (X))
