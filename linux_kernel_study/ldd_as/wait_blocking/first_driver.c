...
static ssize_t my_read(struct file* f, char __user* buf, size_t sz, loff_t* off)
{
  ...
  // variable will be reinitialized for every open operation
  if (*off != 0)
  {
    *off = *off + 1;
  }
  ...
  // static counter will exist till the moudle is removed
  if (count != 0)
    return 0; // return EOF
...
static ssize_t my_write(struct file* f, const char __user* buf, size_t sz,
                        loff_t* off)
{
...
struct file_operations fops = {
    .open = my_open, .release = my_close, .write = my_write, .read = my_read};
